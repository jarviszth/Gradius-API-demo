...
use application\serviceInterface\RadgroupcheckServiceInterface;
use application\model\Radgroupcheck;
class RadgroupcheckService extends DatabaseSupport implements RadgroupcheckServiceInterface
{
    protected $tableName = 'radgroupcheck';
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM radgroupcheck AS radgroupcheck ";

		//where query
        $query .=" WHERE radgroupcheck.`id` IS NOT NULL ";
       ...
       if(!empty($additionalParam)){
		...
		if (is_array($resaultList) || is_object($resaultList)){
            ...
            foreach($resaultList as $obj){
                $singleObj = null;
                $singleObj = new Radgroupcheck($obj);
    ...
    public function findById($id)
    {
        ...
        $query .="FROM radgroupcheck AS radgroupcheck ";
        $query .="WHERE radgroupcheck.`id`=:id ";
		...
		if (is_array($result) || is_object($result)){
            $radgroupcheck = new Radgroupcheck($result);

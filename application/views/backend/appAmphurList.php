...
?>
    <title><?=MessageUtil::getMessage('model_app_amphur')?></title>
    <!-- START PAGE CONTENT -->
    <div class="content">
        <!-- START JUMBOTRON -->
            ...
            <div class="container-fluid container-fixed-lg sm-p-l-20 sm-p-r-20">
                <div class="inner">
                    ...
                    <li><a href="<?=_BASEURL.'dashboard'?>"><?=MessageUtil::getMessage('app_home')?></a></li>
                        <li><a href="" class="active"><?=MessageUtil::getMessage('model_app_amphur')?></a></li>
                    </ul>
                    <!-- END BREADCRUMB -->
                </div>
            </div>
        ...
        <div class="container-fluid container-fixed-lg">
            ...
            <div class="panel panel-default portlet-basic-v">
                <div class="panel-heading separator m-b-10">
                    <div class="panel-title fs-16"><i class="fa fa-list"></i> <?=MessageUtil::getMessage('app_list').' '.MessageUtil::getMessage('model_app_amphur')?></div>
                    ...
                    <div class="btn-group pull-right m-b-10">
                        <a href="<?=_BASEURL.'appamphuradd'?>" class="btn btn-default"><i class="fa fa-plus"></i> <?=MessageUtil::getMessage('app_add_new')?></a>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped" id="basicTable">
                            ...
                            <tr>
                                ...
                                <th><?=MessageUtil::getMessage('model_app_amphur_code')?></th>
                                <th><?=MessageUtil::getMessage('model_app_amphur_name')?></th>
                                <th><?=MessageUtil::getMessage('model_app_amphur_name_eng')?></th>
                                <th><?=MessageUtil::getMessage('model_app_amphur_app_province')?></th>
                            ...
                            if($appAmphurList) {
                                foreach ($appAmphurList AS $appAmphur) {
                                    ?>
                                    <tr id="hide_tr_<?=$appAmphur->getId()?>">
                                        <td >
                                            <div class="checkbox check-danger">
                                                <input type="checkbox" value="<?=$appAmphur->getId()?>" name="check" id="checkbox<?=$appAmphur->getId()?>">
                                                <label for="checkbox<?=$appAmphur->getId()?>"></label>
                                            </div>
                                        </td>
                                        <td >
                                            <p><?=$appAmphur->getCode()?></p>
                                        </td>
                                        <td >
                                            <p><?=$appAmphur->getName()?></p>
                                        </td>
                                        <td >
                                            <p><?=$appAmphur->getNameEng()?></p>
                                        </td>
                                        <td >
                                            <p><?=$appAmphur->getAppProvinceName()?></p>
                                        </td>
                                        <td class="text-right">
                                            <div class="btn-group">
                                                <a href="<?=_BASEURL.'appamphuredit?'.ControllerUtil::genParamId($appAmphur)?>" class="btn btn-default tip" data-toggle="tooltip" data-original-title="<?=MessageUtil::getMessage('app_edit')?>"><i class="fa fa-pencil"></i></a>
                                                <a href="<?=_BASEURL.'appamphurdelete?'.ControllerUtil::genParamId($appAmphur)?>" data-id-hide="hide_tr_<?=$appAmphur->getId()?>" class="btn btn-danger tip app-delete-seleted-confirm"  data-toggle="tooltip" data-original-title="<?=MessageUtil::getMessage('app_delete')?>"  ><i class="fa fa-trash-o"></i></a>
                                            </div>
                                        </td>
                                    </tr>
